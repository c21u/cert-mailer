    ...
    with open(config.distribution_list) as csvfile:
        ...
        for row in reader:
            ...
            if config.mailer == 'sendgrid':
                helpers.sendgrid.send(config, config.introduction_email_subject, body, img, row)
            elif config.mailer == 'mandrill':
                helpers.mandrill.send(config, config.introduction_email_subject, body, img, row)
